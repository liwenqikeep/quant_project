...
- 文档文件名和文档头部的日期**必须**从操作系统获取，禁止凭记忆推算日期。
- 创建文档前执行 `date` 命令或读取系统时间，将实际日期填入文件名和文档头部。
- 日期格式：`yyyy-MM-dd`（如 2026-08-21），**不得**写错为其他日期。

### 6.2 文档创建后必须校验

- 文档写入后**立即**读取文件前 10 行，检查文件名和文档头部的日期是否一致、是否正确。
- 不得以"刚写过没动过"为由跳过校验。
- 发现日期错误必须立即修正，不得拖延到下一轮。
