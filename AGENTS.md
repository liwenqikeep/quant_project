...
凡由 AI 代理生成的**审查、检查、审计类文档**（含代码审查、功能审查、整改记录、回测报告等），必须遵守以下规范：

#### 文件命名规则

- 格式：`yyyy-MM-dd-<简短描述>.md`
- 日期使用**检查完成当天**的公历日期，`yyyy` = 4位年份，`MM` = 2位月份，`dd` = 2位日期
- 示例：
  - `2026-08-19-data-fetch-clean-audit.md` ✅
  - `data_fetch_clean_audit.md` ❌（无日期前缀）
  - `检查报告_20260819.md` ❌（日期格式不符）

#### 文档头部元信息（必须包含）

所有检查类文档**开头**必须包含以下元信息块（Markdown 格式），放在文档标题与正文之间：
...
> **版本：** v0.0.1  
> **审查日期：** 2026-08-19  
> **审查范围：** `backend/quant/data/`、`backend/quant/storage/database.py`  
...
- 后端审查文档统一放在 `docs/` 下（按功能/时间组织，如 `docs/data-fetch-fix-v2/`）
- 跨前后端文档放在根 `docs/`
